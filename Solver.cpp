...
// void LocalSearch::find_move_parallel(Solution &solution)
//{
//     std::vector<std::future<std::pair<std::array<int, 4>, std::array<int, 4>>>> futures;
//
//     int chunk_size = static_cast<int>((solution.conflict_nodes_queue.size() + THREAD_NUM - 1) / THREAD_NUM);// 每个线程处理的区块大小
//
//     for (int i = 0; i < THREAD_NUM; i++)
//     {
//         auto start = i * chunk_size;
//         auto end   = std::min((i + 1) * chunk_size, solution.conflict_nodes_queue.size());
//         if (start < solution.conflict_nodes_queue.size())
//             futures.emplace_back(thread_pool.enqueue(&LocalSearch::find_from_conflict_nodes, this, std::ref(solution), start, end));
//     }
//     int                best_tabu_delta     = INT32_MAX;
//     int                best_non_tabu_delta = INT32_MAX;
//     std::array<int, 4> best_tabu_move      = {-1, -1, -1, INT32_MAX};
//     std::array<int, 4> best_non_tabu_move  = {-1, -1, -1, INT32_MAX};
//     int                tabu_move_num       = 0;
//     int                non_tabu_move_num   = 0;
//
//     for (auto &future: futures)
//     {
//         auto [tabu_move, non_tabu_move] = future.get();
//         if (tabu_move.at(0) != -1)
//         {
//             if (tabu_move.at(3) < best_tabu_delta)
//             {
//                 best_tabu_delta = tabu_move.at(3);
//                 best_tabu_move  = tabu_move;
//                 tabu_move_num   = 1;
//             } else if (tabu_move.at(3) == best_tabu_delta)
//             {
//                 tabu_move_num++;
//                 if (_random_generator.rand(tabu_move_num) == 0)
//                 {
...
//             }
//         }
//         if (non_tabu_move.at(0) != -1)
//         {
//             if (non_tabu_move.at(3) < best_non_tabu_delta)
//             {
//                 best_non_tabu_delta = non_tabu_move.at(3);
//                 best_non_tabu_move  = non_tabu_move;
//                 non_tabu_move_num   = 1;
//             } else if (non_tabu_move.at(3) == best_non_tabu_delta)
//             {
//                 non_tabu_move_num++;
//                 if (_random_generator.rand(non_tabu_move_num) == 0)
//                 {
...
//         }
//     }
//     if (best_tabu_delta < best_conflict_num - solution.conflict_num && best_tabu_delta < best_non_tabu_delta)
//     {
//         move = {best_tabu_move.at(0), best_tabu_move.at(1), best_tabu_move.at(2), best_tabu_delta};
//     } else
//     {
//         move = {best_non_tabu_move.at(0), best_non_tabu_move.at(1), best_non_tabu_move.at(2), best_non_tabu_delta};
//     }
// }
